...
```{r}
ins_eval <- read.csv("https://raw.githubusercontent.com/Riteshlohiya/Data621-Assignment-4/master/insurance-evaluation-data.csv")
var_class <- data.frame(Class = rep(NA, ncol(ins_eval) - 1), Levels = rep(NA, ncol(ins_eval) - 1), stringsAsFactors = FALSE, check.names = FALSE, row.names = names(ins_eval)[-1])
for(i in 2:ncol(ins_eval)) {
  var_class[i - 1, 1] <- class(ins_eval[, i])
  var_class[i - 1, 2] <- ifelse(length(levels(ins_eval[, i])) == 0, '-', length(levels(ins_eval[, i])))
}
pander(var_class)
